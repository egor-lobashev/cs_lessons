...
bool modeChoice(sf::RenderWindow *window, bool *quit)
{
	...
	sf::Text text("press o for one player\npress t for two players", font, 50);
...
int main()
{
	...
	while (not (gameOver or quit))
	{
		...
		if (ball1.hp == 0)
		{
			sf::Font font;
			font.loadFromFile("arial.ttf");
			sf::Text text("player 2 is winner", font, 50);
			text.setFont(font);
		...
		if (ball2.hp == 0)
		{
			sf::Font font;
			font.loadFromFile("arial.ttf");
			sf::Text text("player 1 is winner", font, 50);
			text.setFont(font);
			text.setPosition(100,200);
			window.draw(text);
			gameOver = true;
